...
export function GoogleButton({ label = "Continue with Google" }: { label?: string }) {
  ...
  return (
    <button
      type="button"
      onClick={start}
      className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white p-3 font-semibold text-slate-700 transition-colors hover:bg-slate-50"
    >
      <GoogleIcon />
      {label}
    </button>
