...
- **`GenerateStream()` / `GenerateStreamEvents()` (P1.1, 11 Sep 2026):** request memuat `stream:true`; `bufio.Reader` membaca SSE provider incremental. `GenerateStreamEvents` meneruskan tiap `delta.content` provider nyata inline dan mengirim sinyal klasifikasi `tool_call` tanpa membocorkan argumen parsial; wrapper `GenerateStream` menjaga interface callback teks lama. `AIService.generateWithToolLoopStream` memakai klasifikasi itu: teks round final diteruskan langsung ke SSE `delta`, sedangkan content pada/ setelah chunk tool-call tidak ditampilkan (guard BUG-12) dan argumen tool tetap diakumulasi privat sebelum eksekusi. Error setelah delta nyata tetap terminal (`error`, tanpa persist/done); disconnect membatalkan context provider. Fallback lokal tetap boleh menghasilkan teks, tetapi tidak menandai telemetry `first_delta` provider.
