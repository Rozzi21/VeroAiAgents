...
> Audit GenUI Travel Package (read-only): 6 Sep 2026 — kartu rekomendasi hilang saat reload (B-GENUI-2), tak ada id pesan stabil, tak ada recovery stream putus (B-GENUI-1). Laporan: `docs/ai/GENUI_TRAVEL_PACKAGE_AUDIT.md`. **Fondasi persistensi SUDAH DIKERJAKAN (6 Sep 2026)**: metadata rekomendasi dipersist di `chat_messages.recommendation` (jsonb), `message_id` stabil dikirim via `done`/history, reload merekonstruksi kartu dari DB tanpa `search_trips`/LLM. **Limitasi tersisa**: stream yang putus SEBELUM persist tidak meninggalkan jejak dan memang tidak boleh direkonstruksi klien. Rekonsiliasi stream putus sesudah persist sudah otomatis sejak 9 Sep 2026; lihat audit B-GENUI-1 di bawah. **Audit di-refresh 9 Sep 2026** (dokumen yang sama). **B-GENUI-3, B-GENUI-4, B-GENUI-5, dan B-GENUI-6 SUDAH DIPERBAIKI (9 Sep 2026)**: aksi "Pilih Paket Ini" tetap backend-authoritative; alternatif eksplisit tetap menghasilkan set baru sambil menjaga seleksi; pricing AIW-5 mengalir dari `search_trips` ke `ChatResult`/JSONB/SSE/history lalu dirender kartu; dan pembacaan draft dari tool mati `update_order_draft` telah dihapus tanpa mengganti kontrak aktif. Temuan tersisa — B-GENUI-7 (sejak fix BUG-12 semua round tool memakai `GenerateStream(..., nil)` sehingga event `delta` praktis hanya mengalir saat `MaxToolCallRounds` habis; jalur umum tidak streaming live, frontend mengetik via `TypingText`), B-GENUI-8 (`order_gate` + `workflow` tidak dipersist — hilang saat reload; hanya rekomendasi yang pulih).

> Audit stream recovery chat SSE — B-GENUI-1 residual (read-only): 9 Sep 2026 — analisis eksak semua titik gagal saat stream putus sebelum `done` (termasuk gap EOF-tanpa-`done` di `streamChat` yang tidak memicu callback apa pun, dan pelebaran jendela race oleh proxy SSE Next.js), timing persistensi (`finalizeChat` INSERT sebelum `done`), ketersediaan `message_id` (hanya via `done`), plus analisis 5 race condition (A persisted/done-hilang, B belum-persist, C late-event, D sukses, E alternatif-4C) beserta mekanisme recovery minimal (reconcile history sekali di onError — bukan resume stream, tanpa retry LLM/`search_trips`). Laporan: `docs/ai/GENUI_STREAM_RECOVERY_AUDIT.md`. **SUDAH DIIMPLEMENTASIKAN DAN DI-HARDEN (9 Sep 2026)** di frontend: EOF tanpa `done` memicu `onError` tepat sekali; UI mengambil history tepat sekali dan mengganti hanya placeholder gagal lewat helper murni `reconcileFailedTurn`, dedup `message_id`, menjaga rekomendasi lama/alternatif serta sinkronisasi `selected_trip_id`. Guard terminal menolak late `done`/callback berulang saat recovery; replacement dan rekonsiliasi berulang idempoten untuk rerender/Strict Mode. Bila assistant belum persisted, error lokal dipertahankan tanpa polling atau retry chat.
...
- **BUG PKCE double-hash (ditemukan + DIPERBAIKI 9 Sep 2026)** — SEMUA login Google asli gagal `invalid_grant "Invalid code verifier."` sejak commit PKCE `f7968a1`. Akar masalah: `oauth2.S256ChallengeOption` di `golang.org/x/oauth2` **menerima code_verifier mentah dan menghash S256 sendiri** (`S256ChallengeFromVerifier`, lihat `pkce.go` library). `AuthCodeURLForRedirect` (`internal/auth/google.go`) malah mengoper hasil `pkceS256Challenge(codeVerifier)` dari `google_oauth_service.go` — challenge ter-hash DUA KALI (`S256(S256(verifier))`), sedangkan exchange mengirim verifier mentah (`VerifierOption`). Google menghitung `S256(verifier)` ≠ challenge terikat → reject. Terbukti live: probe `GET /auth/google` + baca `o_auth_states` menunjukkan `challenge_sent == S256(S256(verifier_tersimpan))` byte-for-byte. Mocked E2E tidak menangkap ini karena fake token server tidak memvalidasi PKCE. **Fix**: `StartLogin` kini mengoper `codeVerifier` mentah ke `AuthCodeURLForRedirect`; `pkceS256Challenge` dihapus; parameter `AuthCodeURL`/`AuthCodeURLForRedirect` direname `codeChallenge`→`codeVerifier` agar kontrak tak ambigu. Regresi dikunci: `runFlow` di `google_oauth_callback_test.go` mengassert `code_challenge == oauth2.S256ChallengeFromVerifier(verifier tersimpan)` + method S256; `TestPKCE_S256Challenge` mengunci vektor RFC 7636 terhadap `S256ChallengeFromVerifier`. Diverifikasi live: authorize URL kini memuat `code_challenge == S256(verifier mentah)` (MATCH). Catatan sampingan: GORM memakai tabel `o_auth_states` (tanpa `TableName` override), sedangkan migrasi SQL/docs menyebut `oauth_states` — dua tabel bisa ada; aplikasi konsisten memakai `o_auth_states`.
