...
| GET | `/api/v1/chat/history` | 🔓 guest cookie | Pulihkan history guest aktif; session identifier tidak diterima dari request dan tidak dikembalikan. Sejak 6 Sep 2026 tiap pesan membawa `id` (message id stabil) + `recommendation?` (metadata kartu paket yang dipersist, bila ada). Paket rekomendasi memakai field model `Trip`; sejak B-GENUI-5 (9 Sep 2026), harga dewasa/anak, flag + nominal diskon, destination, dan duration dari `search_trips` dipertahankan di metadata ini dan sama dengan payload `done`. Payload top-level juga membawa `selected_trip_id?` agar reload memulihkan status kartu terpilih (B-GENUI-3) |
