...
| `Sidebar` | `frontend/src/components/layout/Sidebar.tsx` | Navigasi kiri (sebagian link masih placeholder `href="#"`); blok bawah memakai `AuthStatus` (9 Sep 2026): identitas user + logout, atau link Login/Create Account untuk tamu |
...
- **`GoogleButton.tsx`** — memulai Authorization Code flow lewat **full-page navigation** (`window.location.href = /api/v1/auth/google?return_to=<pathSaatIni>`; path `/google/login` direname ke `/google` 24 Agu 2026), BUKAN `apiFetch` (browser harus mengikuti redirect consent Google). `return_to` = `usePathname()` + query saat ini agar user kembali ke halaman asal; sejak 9 Sep 2026 query dibersihkan dulu lewat `sanitizeOAuthReturnQuery()` (param one-shot `auth_error`/`google_linked` dibuang agar error basi tidak round-trip ke attempt berikutnya). Dipakai di `/login`, `/register`, dan guest-gate `trip/[id]` (menggantikan tombol placeholder disabled). Ikon Google inline SVG (tanpa dependency baru, patuh coding-rules §2.7).
- **`OAuthReceiver.tsx`** — client component yang membaca `#access_token=...` dari **URL fragment** redirect callback backend (fragment tidak pernah dikirim ke server), memanggil `setCustomerAccessToken`, membersihkan hash via `history.replaceState`, lalu reload path saat ini. Sejak 9 Sep 2026: sukses dari `/login`/`/register` redirect ke `/` (`postOAuthSuccessPath()`, konsisten dengan login password); path bersih juga di-sanitize (tanpa `auth_error` basi); bila `setCustomerAccessToken` menolak token valid (storage penuh/diblokir) permukaan error eksplisit via `onError` dan login TIDAK dianggap sukses. Juga membaca `?auth_error=<code>` dan memetakannya ke pesan ramah via `oauthErrorMessage()` (27 Agu 2026): `access_denied` → "Google sign-in was cancelled..." (user batal / Google menolak consent), `start_failed` → gagal memulai (backend error), `missing_params`/`authentication_failed` → state OAuth invalid/kedaluwarsa, gagal tukar code, atau verifikasi id_token gagal (callback error), `account_exists_link_required`/`google_identity_taken` → konflik akun; kode tak dikenal → pesan generik. Pesan diteruskan via prop `onError`. Dipasang di `/login`, `/register`, `trip/[id]`, dan `ChatInterface` — sejak 9 Sep 2026 SEMUA pemasangan meneruskan `onError` (trip/chat menampilkan banner error, tidak lagi senyap).
- **`AuthStatus.tsx`** **(9 Sep 2026)** — client component di root `Sidebar` (menggantikan blok statis "My Profile") sebagai satu-satunya sumber status login UI customer: saat mount memanggil `ensureCustomerSession()` lalu `fetchCurrentCustomer()` (`GET /api/v1/auth/me`, helper baru di `api.ts`). Anonymous → link Login/Create Account; authenticated → nama/email + tombol logout yang memanggil `customerLogout()` (revoke sesi server + clear token lokal) lalu `window.location.href = "/login"` (full-page navigation membuang state auth in-memory semua komponen).
...
- **`ensureCustomerSession(): Promise<"active" | "anonymous">`** — menjamin access token bisa dipakai. Bila token sudah tersimpan → langsung `"active"`. Bila tidak (mis. access token 15-menit sudah kedaluwarsa / tab baru), ia menukar **cookie refresh HttpOnly** via `POST /api/v1/auth/refresh` (rotasi atomik, sama seperti password login) lalu menyimpan token baru. Balas `"anonymous"` bila tidak ada sesi (belum login / sesi di-revoke / expired / reuse). `refreshInFlight` tetap dedup caller satu tab. Sejak 9 Sep 2026, `frontend/src/lib/refreshCoordinator.ts` juga serialisasi lintas-tab: primary `navigator.locks` (`Web Locks API`, auto-release saat tab crash/tutup); fallback mutex localStorage `vero_customer_refresh_lock` dengan TTL stale 40 dtk (> timeout request 35 dtk) + tunggu event `storage` (bukan polling). Tab menunggu re-check access token shared setelah lock mendapat hasil, jadi TIDAK mengirim refresh kedua. Refresh 401 menulis marker outcome `anonymous`; tab lain tidak mengulang refresh, semua converge logout. Refresh token TIDAK pernah masuk localStorage/sessionStorage — hanya cookie HttpOnly yang dipakai.
- **`customerLogout(): Promise<void>`** — sign-out NYATA: `POST /api/v1/auth/logout` membaca cookie refresh dan me-revoke JTI-nya di server (persis logout password), lalu membersihkan access token lokal. Sejak 9 Sep 2026 juga menulis marker `anonymous` lintas-tab: token hasil refresh yang selesai SETELAH logout dibuang, tidak dapat menghidupkan sesi lagi. Aman dipanggil saat sudah anonymous.
...
**Telemetry chat P0.1 (10 Sep 2026):** `frontend/src/lib/chatTelemetry.ts` membuat satu random `X-Request-ID` per submit dan event elapsed-time `submit`, `auth-ready`, `request-start`, `response-headers`, `first-sse-event`, `first-delta`, `first-react-paint`, `done`, `recommendation-card-rendered`. `streamChat()` mengirim ID itu; proxy `app/api/v1/chat/route.ts` meneruskannya ke backend dan mengekspos response `X-Request-ID` kembali. Sink default `console.info("chat_telemetry", ...)` hanya berisi event, request ID, elapsed ms, status—tanpa prompt, user/session, email, authorization/token, OAuth secret, atau payload. Error sink ditelan sehingga behavior chat tidak berubah.
...
**True streaming P1.1 (11 Sep 2026):** parser `streamChat()` memakai `TextDecoder({stream:true})`, memelihara trailing event lintas network chunk, menerima LF/CRLF, banyak event per chunk, event terbelah, multibyte UTF-8 terbelah, dan memproses event final tanpa delimiter saat EOF. Event `delta` langsung masuk scheduler render; event `recommendation` memasang GenUI pada message aktif segera, tanpa menunggu text selesai atau `done`. Live request tidak memakai `TypingText`/delay lokal; `done` hanya finalisasi content, stable `message_id`, workflow/order gate, dan metadata persisted. Next route meneruskan `request.signal` ke fetch backend agar disconnect browser membatalkan provider work.
...
| Dependencies npm | `clsx`, `lucide-react` (^1.18), `next`, `react`, `react-dom`, `tailwind-merge` — **tanpa** library animasi eksternal; chat live mengikuti provider delta tanpa delay buatan |
